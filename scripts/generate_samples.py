...
calls the real Claude API (and, if a ticker is given, real Yahoo Finance)
and costs real money per run. Run this by hand against any source
document to produce one example PDF:

    ANTHROPIC_API_KEY=... python scripts/generate_samples.py \\
        "Example Corp" local-input/document.pdf example_corp_report.pdf [ticker]

The optional 4th argument is a stock ticker (e.g. JSWENERGY.NS) to
include a live Live Market Data section, matching the web form's
optional ticker field - omit it to auto-match from the company name, or
pass an empty string to skip market-data enrichment entirely.
...
from app.market_data.lookup import get_market_data  # import the best-effort live market-data lookup
...
USAGE = (  # multi-line usage string shown on argument errors
    ...
    '"<Company Name>" <source_path> <output_filename>.pdf [ticker]'  # second half showing the three required + one optional positional args
)


def generate_one(company_name: str, source_path: Path, output_filename: str, ticker: str) -> None:  # run the full pipeline for one source document
    ...
    market_data = get_market_data(company_name, ticker)  # best-effort live market-data lookup, same call main.py's route makes
    print(f"  market data: {'found (' + market_data.matched_ticker + ')' if market_data.found else 'not available'}")  # report what the lookup actually resolved, for manual verification
    chart_images = build_charts(report_data.chart_series)  # render each chart series into a base64 PNG data URI
    context = map_to_template_context(report_data, chart_images, market_data)  # map ReportData + chart images + market data into the template's context dict
...
def main() -> None:  # entry point that parses argv and generates one example PDF
    """Parse company name, source path, output filename, and optional
    ticker from argv, then generate exactly one example PDF (call this
    once per document you want to produce - the script keeps no document
    list of its own)."""
    if len(sys.argv) not in (4, 5):  # expect 3 required positional args, plus one optional ticker
    ...
    ticker = sys.argv[4] if len(sys.argv) == 5 else ""  # optional 4th arg: ticker for live market-data enrichment, blank if omitted
    ...
    if not source_path.is_file():  # guard: a directory path would fail obscurely inside read_bytes()
        ...
        sys.exit(1)  # exit non-zero since no output can be produced
    generate_one(company_name, source_path, output_filename, ticker)  # run the pipeline for the single requested document
